...
python manage.py seed_admin                  # create the super admin: admin / admin123
python manage.py seed_admin --reset-password # reset admin password back to admin123
python manage.py seed_workflow               # create the default 3-stage workflow + a default teacher
python manage.py createsuperuser             # create a Django auth superuser (orthogonal to role=admin)
...
- **[apps/users/](backend/apps/users/)** — custom `User` model with `role ∈ {student, teacher, admin}`, plus `Major` and `SchoolClass` ([users/models.py](backend/apps/users/models.py)).
  - **Role hierarchy**: `admin` manages teachers + majors + global Dify config; `teacher` manages own `SchoolClass` rows and the `student` users inside them. A teacher may own many classes; a class belongs to exactly one teacher.
  - **No public registration.** `/api/auth/register/` was removed. Accounts are created top-down: `seed_admin` makes the super admin; admin creates teachers via `/api/admin/teachers/`; teachers create students one-at-a-time or via CSV import. JWT auth lives at `/api/auth/{login,profile,change-password}/`.
  - A `post_save` signal in [users/signals.py](backend/apps/users/signals.py) auto-creates a `DifyUserMapping` **only for `role='student'`** (admins/teachers don't need a Dify ID).
  - Admin endpoints in [views_admin.py](backend/apps/users/views_admin.py), routed under `/api/admin/` ([urls_admin.py](backend/apps/users/urls_admin.py)).
  - Teacher class/student endpoints in [views_teacher.py](backend/apps/users/views_teacher.py), routed under `/api/teacher/` ([urls_teacher.py](backend/apps/users/urls_teacher.py)). To avoid colliding with scoring's `/api/teacher/students/...`, these endpoints use `/api/teacher/class-students/...`.
  - **CSV import** ([views_teacher.py](backend/apps/users/views_teacher.py) `teacher_class_students_import_view`): headers must be `学号,密码,专业,班别,姓名,性别`; required cells `学号/姓名`; password blank → defaults to 学号; gender accepts `男/女/其他/空`. **Strict transaction**: any row error aborts the entire batch (no partial inserts).
...
- **[apps/dify_integration/](backend/apps/dify_integration/)** — Dify API wrapper plus workflow/stage definitions.
  ...
  - [models.py](backend/apps/dify_integration/models.py): `DifyConfig` (singleton — **admin-only** read/write via `/api/admin/dify-config/`), `DifyUserMapping`, `LearningWorkflow` (teacher FK is `PROTECT` so teachers can't be deleted while owning a workflow), `WorkflowStage`. A workflow is an ordered list of stages with `stage_key`, `expected_min_messages`, `expected_min_minutes`, and `weight`.
...
- `/api/auth/` → users (login / profile / change-password)
- `/api/admin/` → super-admin only: teachers CRUD, majors CRUD, Dify config
- `/api/chat/` → dify_integration (student chat + sync trigger)
- `/api/logs/` → chat_logs
- `/api/teacher/` → mixed: scoring views (`apps/scoring/urls.py`) + class/student management (`apps/users/urls_teacher.py`). Both share the prefix; routes don't overlap.
...
- [api/admin.js](frontend/src/api/admin.js) — admin-only: teacher CRUD, major CRUD. Dify config getters/setters still live in `api/teacher.js` for historical reasons but hit `/api/admin/dify-config/`.
- [api/teacherClasses.js](frontend/src/api/teacherClasses.js) — teacher class/student CRUD + CSV import.
- [router/index.js](frontend/src/router/index.js) — auth guard reads `localStorage.access_token`. Role guard: `meta.role: 'teacher'` accepts teacher+admin; `meta.role: 'admin'` is admin-only; `meta.role: 'student'` redirects non-students to their dashboard.
- [stores/user.js](frontend/src/stores/user.js) — exposes `role`, `isStudent`, `isTeacher`, `isAdmin`, `isTeacherOrAdmin`. **Note**: `isTeacher` is strict — it does NOT include admins. Use `isTeacherOrAdmin` if you need both.
- [views/admin/](frontend/src/views/admin/) — `Dashboard`, `TeacherManage`, `MajorManage`, `DifyConfig`.
- [views/teacher/](frontend/src/views/teacher/) — `Dashboard`, `ClassManage`, `ClassStudents` (with CSV import), `StudentDetail`, `ScoreReview`, `Settings` (scoring + stage thresholds; Dify config removed).
- [views/](frontend/src/views/) — `Login` (no `Register` anymore), `MainLayout` (top nav branches by role), `ChatView` (student).
